  ...
  return (
    <AppLayout title="Hiring Intelligence" lastUpdated={lastUpdated}>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        ...
        <div className="bg-slate-900/60 border border-slate-800 rounded-xl p-4">
          <p className="text-2xl font-bold text-white">
            {jobs.filter((j) => j.posted_at && new Date(j.posted_at) > new Date(Date.now() - 7 * 24 * 60 * 60 * 1000)).length}
          </p>
          <p className="text-xs text-slate-400 mt-0.5">Posted last 7 days</p>
        </div>
      </div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-slate-800">
                ...
                <th className="text-right px-4 py-3 text-xs font-semibold text-slate-400 uppercase tracking-wider">Posted</th>
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-4 py-3 text-right text-xs text-slate-500">
                        {job.posted_at ? formatDate(job.posted_at) : formatRelativeTime(job.scraped_at)}
                      </td>
